...
const previousComing = loadGlobal(OUT, "WTOP_COMING_SEASONS") || [];
...
// A folder's name, which works even when the folder is empty (GetSessions
// only reveals it through its videos). Used for "coming soon" seasons.
async function folderName(folderID) {
  const res = await fetch(HOST + "/Panopto/Services/Data.svc/GetFolderInfo", {
    method: "POST",
  ...
  if (!d || !d.Name) throw new Error("no folder name");
  return d.Name;
}
...
const report = [];
let failures = 0;
// Empty folders: a new season that hasn't had an episode yet
const coming = [];
...
for (const folder of folders) {
  ...
  try {
    ...
    episodes.push(...kept);
    coming.push(...previousComing.filter((c) => c.folder === folder));
    continue;
  }
  if (!rows.length) {
    // Name it once and remember it; folder names rarely change
    const known = previousComing.find((c) => c.folder === folder);
    let name = known && known.name;
    if (!name) {
      try { name = await folderName(folder); } catch (err) { console.warn(`! Folder ${folder}: no name (${err.message})`); }
    }
    const show = name && showFor(name);
    if (show) coming.push({ show: show.id, season: seasonOf(name, new Intl.DateTimeFormat("en-CA", { timeZone: "America/New_York" }).format(new Date())), name, folder });
    else if (name) console.warn(`! Empty folder "${name}" matches no show`);
    console.log(`Folder ${folder}: empty${name ? " (" + name + ")" : ""}`);
    continue;
  }
...
writeFileSync(OUT,
  ...
  "window.WTOP_COMING_SEASONS = " + JSON.stringify(coming.sort((a, b) => a.show.localeCompare(b.show) || a.folder.localeCompare(b.folder)), null, 2) + ";\n");
