...
Newscasts in the guide: Cablecast lists them all as "WTOP-10 NEWS". `parseStationCsv` relabels them Nightly News (and its reruns), except a Friday 9:30 AM listing, which is Rise and Shine Oswego (show id `rise-and-shine`, formerly Morning News; airs only then, `airs` Fridays 9:30 AM; CANONICAL_TITLES maps "morning news", "raso" and "rise and shine" to it, and it matches Panopto titles with any of those). So the mini guide, Live tab and program guide link and label them correctly. The guide's trailing-date split also accepts a hyphen before the date ("...vs Plattsburgh-11-4-2022").

## Panopto source

Titles are often raw file names ("hockeytalk9-18-26l", "OswegoTaskmaster_s1_e1 9-17-26", "U.S.S.S2.EP.1 9-17-26"). When a title matches no show, the build uses the show named by the folder (`showFor(row.FolderName)`) and drops the title, so the card shows "Episode N" or the date. Title matches still win, so shared folders (News) route by title. `episodeOf` accepts s1_e1 / S2.EP.1 / s5 ep 1 / episode 2; dates may touch letters; a trailing all-caps FINAL/EXPORT/V2, or a leftover that's only a version note in any spelling (finalcut, final_cut, Final Export, draft, v2), is dropped.

`Data.svc/GetFolderInfo` (POST `{folderID}`) returns a folder's `Name` and `ParentName` without a login, even when the folder is empty (GetSessions only reveals `FolderName` through its sessions). Fall 2026 show folders sit under a parent folder named "Fall 2026" (4ed201f6-30de-40c4-a4c5-b48e0005d56a); listing its subfolders still needs a login, so each folder link is listed in `panoptoFolders`. An empty public folder and a private one look the same from outside (0 sessions). The build uses GetFolderInfo for empty folders: the name maps to a show (`showFor`) and season (`seasonOf`), written to `episodes.js` as `window.WTOP_COMING_SEASONS` ({show, season, name, folder}); names are reused from the previous run, so Panopto is asked once per folder. The show page hero adds a red "Season 6 coming soon" tag (`comingFor`), and the first row is labeled "· latest season" instead of "· current season" while one is pending. A folder whose GetSessions fails keeps its previous coming-soon entry. There are no hand-entered episodes left: Storm Team 10 Live's 9/15/26 episode now comes from its folder.
...
- Rows: "Latest episodes", on Home "Latest Live Sports" (newest episodes of `thumbFind` game shows), "Shows", then one row per show once it has 2+ episodes. Shows (tiles and rows) are ordered by their newest episode, the same order as "Latest episodes" (first index in the sorted `episodes` array). Each show row's heading is a link to the show page (`.row-link`, a real `?show=` href; plain clicks use `setFilter`, modified clicks open normally); the Latest rows' red show names are not links. Each show row starting on its newest season; a multi-season show's row has a season dropdown that refills just that row's track (`addRow` returns its section). The menu filters by category. A show page lists every season as its own row: current season (tagged "· current season" when there are others), then "Upcoming on WTOP-10", then earlier seasons, then "More from WTOP-10". No season in the address. Cards show run time when known.
